...
public class ExtraerCRPS
{
	public static void main(String[] args)
	    throws B2BException
	{
		CRPSBusiness lcrpsb_business;
		Date         ld_ciclo;
		Date         ld_fin;
		Date         ld_inicio;
		Date         ld_max;
		...
		lcrpsb_business     = new CRPSBusiness();
		ld_inicio           = new Date();
...
//		ld_max              = DateUtils.getDate(ld_inicio, Calendar.DATE, 1, false);
		...
		while(ld_tmp.before(ld_max))
		{
			try
			{
				lcrpsb_business.generarCRPS(ld_tmp, "garias_conciliaciones", "192.168.100.102");
			}

...
import com.bachue.snr.prosnr21.web.bean.BeanMenu;
...
public class BeanLogin extends BaseBean implements Serializable
{
	...
	public String getVersionActual()
	{
		if(!StringUtils.isValidString(is_versionActual))
		{
			...
			{
				li_inicio = ls_version.indexOf("SNRCMFEAR_");
	    ...
	    throws B2BException
	{
		...
		PrimeFaces.current().ajax().update("idFMenu:idMMenuPrincipal");
	}
	    ...
	    throws IOException
	{
		...
		try
		{
			...
			if(StringUtils.isValidString(ls_idUsuario))
			{
				lu_usuario = icr_conciliacionesRemote.findUserById(new Usuario(ls_idUsuario));
					...
					{
						{
							...
							lco_opciones = icr_conciliacionesRemote.cargarOpcionesMenu(
								    getUsuario(), ComponentesCommon.LMS_COMPONENTE, getLocalIpAddress(),
								    getRemoteIpAddress()
								);
	...
	/**
	 * Método para redirigir al home de bachué
	 * @throws IOException
	 */
	private void redirigirHome()
	    throws IOException
	{
		FacesContext.getCurrentInstance().getExternalContext().redirect("https://localhost:7001/SNRWEB/pages/home.jsf");
	}
}

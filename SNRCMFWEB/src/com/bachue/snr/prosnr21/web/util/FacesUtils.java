...
public class FacesUtils implements Serializable
{
	...
	{
		HttpServletRequest lhs_request;

		lhs_request = getRequest();

		return (lhs_request != null) ? lhs_request.getServletContext() : null;
	}
	    ...
	    throws B2BException
	{
		...
		if((acu_bean != null) && StringUtils.isValidString(as_beanNameCommon))
		{
			...
			lo_bean         = (Object)lfc_context.getApplication()
					                                 .evaluateExpressionGet(lfc_context, as_beanNameCommon, acu_bean);

			if((lo_bean == null) || !acu_bean.isInstance(lo_bean))

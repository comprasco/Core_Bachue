...
/**
 * Clase que contiene todos las propiedades LiquidacionesHistoricosFolioDAO.
 *
 * @author  Carlos Calderón
 * Fecha de Creacion: 8/06/2020
 */
public class LiquidacionesHistoricosFolioDAO extends AuditoriaDao
{
	/** Constante cs_FIND_ALL_BY_TURNO_ACTUALIZADO. */
	private static final String cs_FIND_ALL_BY_TURNO_ACTUALIZADO = "SELECT * FROM SDB_VW_LIQUIDACIONES_HISTORICOS_FOLIO WHERE TURNO_ACTUALIZADO = ? ORDER BY ACTO DESC";
	...
	/**
	 * Find all by turno actualizado folio.
	 *
	 * @param as_turnoActualizado de as turno actualizado
	 * @return el valor de collection
	 * @throws B2BException Objeto de tipo B2BException, se produce cuando se encuentra algun error controlado.
	 */
	public Collection<LiquidacionesHistoricosFolio> findAllByTurnoActualizadoFolio(String as_turnoActualizado)
	    throws B2BException
	{
		Collection<LiquidacionesHistoricosFolio> lclhf_liquidacionesHistoricos;
		PreparedStatement                        lps_ps;
		ResultSet                                lrs_rs;

		lclhf_liquidacionesHistoricos     = new ArrayList<LiquidacionesHistoricosFolio>();
		lps_ps                            = null;
		lrs_rs                            = null;
		...
		if(StringUtils.isValidString(as_turnoActualizado))
		{
			try
			{
				...
				while(lrs_rs.next())
					lclhf_liquidacionesHistoricos.add(getLiquidacionHistoricosFolio(lrs_rs));
			}
			catch(SQLException lse_e)
			{
				logError(this, "findAllByTurnoActualizadoFolio", lse_e);
		...
		if(lclhf_liquidacionesHistoricos.isEmpty())
			lclhf_liquidacionesHistoricos = null;

		return lclhf_liquidacionesHistoricos;
	}
...
//			lap_admPagos.setRecibo(getBigInteger(ars_rs, "RECIBO"));
	 ...
	 * @throws SQLException cuando se produce algun error en el proceso
	 */
	private LiquidacionesHistoricosFolio getLiquidacionHistoricosFolio(ResultSet ars_rs)
	    throws SQLException
	{
		LiquidacionesHistoricosFolio lr_datos;

		lr_datos = new LiquidacionesHistoricosFolio();

		lr_datos.setIdCirculo(ars_rs.getString("ID_CIRCULO"));
		lr_datos.setNombre(ars_rs.getString("NOMBRE_CIRCULO"));

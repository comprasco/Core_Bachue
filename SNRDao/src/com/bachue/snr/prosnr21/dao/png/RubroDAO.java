...
/**
 * Clase que contiene todos las consultas de la tabla SDB_PGN_RUBRO
 * @author dbeltran
 *
 */
public class RubroDAO extends AuditoriaDao
{
	/**Constante cs_INSERT*/
	private static final String cs_INSERT = "INSERT INTO SDB_PGN_RUBRO (ID_RUBRO, NOMBRE, ACTIVO, ID_USUARIO_CREACION, FECHA_CREACION, IP_CREACION, NUMERO_RUBRO) VALUES (?, ?, ?, ?, SYSTIMESTAMP, ?, ?)";

	/**Constante cs_UPDATE*/
	private static final String cs_UPDATE = "UPDATE SDB_PGN_RUBRO SET NOMBRE = ?, ACTIVO = ?, ID_USUARIO_MODIFICACION = ?, FECHA_MODIFICACION = SYSTIMESTAMP, IP_MODIFICACION = ?, NUMERO_RUBRO = ? WHERE ID_RUBRO = ?";

	/**Constante cs_FIND_ALL*/
	private static final String cs_FIND_ALL = "SELECT * FROM SDB_PGN_RUBRO";

	/**Constante cs_FIND_ALL*/
	private static final String cs_FIND_ALL_ACTIVO = "SELECT * FROM SDB_PGN_RUBRO WHERE ACTIVO = 'S'";

	/**Constante cs_FIND_BY_ID*/
	private static final String cs_FIND_BY_ID = "SELECT * FROM SDB_PGN_RUBRO WHERE ID_RUBRO = ?";

	/**Constante cs_FIND_BY_NOMBRE*/
	private static final String cs_FIND_BY_NOMBRE = "SELECT * FROM SDB_PGN_RUBRO WHERE NOMBRE = ?";

	/**Constante cs_FIND_SECUENCE*/
	private static final String cs_FIND_SECUENCE = "SELECT SEC_PGN_RUBRO_ID_RUBRO.NEXTVAL FROM DUAL";

	/**
	 * Metodo para encontrar todos los registros existentes en la tabla SDB_PGN_RUBRO
	 * @return
	 * @throws B2BException
	 */
	public Collection<Rubro> findAll()
	    throws B2BException
	{
		Collection<Rubro> lcpr_cpr;
		PreparedStatement lps_ps;
		ResultSet         lrs_rs;

		lcpr_cpr     = new ArrayList<Rubro>();
		lps_ps       = null;
		lrs_rs       = null;
	 ...
	 * @throws B2BException
	 */
	public Collection<Rubro> findAllActivo()
	    throws B2BException
	{
		Collection<Rubro> lcpr_cpr;
		PreparedStatement lps_ps;
	...
	/**
	 *Metodo para encontrar  los registros existentes en la tabla SDB_PGN_RUBRO buscados por ID_RUBRO
	 * @param atd_param
	 * @return
	 * @throws B2BException
	 */
	public Rubro findById(Rubro atd_param)
	    throws B2BException
	{
		return ((atd_param != null) ? findById(atd_param.getIdRubro()) : null);
	}
	 ...
	 * @throws B2BException
	 */
	public Rubro findById(String as_param)
	    throws B2BException
	{
		Rubro ltd_return;
		ltd_return = null;
	 ...
	 * @throws B2BException
	 */
	public Rubro findByNombre(String as_param)
	    throws B2BException
	{
		Rubro ltd_return;
		ltd_return = null;
		...
		if(StringUtils.isValidString(as_param))
		{
			...
			try
			{
				lps_ps = getConnection().prepareStatement(cs_FIND_BY_NOMBRE);
			...
			{
				logError(this, "findByNombre", lse_e);
	...
	/**
	 * Metodo para insertar un registro en la base de datos de la tabla SDB_PGN_RUBRO
	 */
	public void insert(Rubro atd_param)
	    throws B2BException
	{
		if(atd_param != null)
		{
			...
			try
			{
				...
				if(lrs_rs.next())
				{
					...
					if((lo_o != null) && lo_o instanceof BigDecimal)
						atd_param.setIdRubro(lo_o.toString());
					else
				...
				lps_ps.setString(li_column++, atd_param.getNumeroRubro());
	...
	/**
	 * Metodo para actualizar un registro en la base de datos de la tabla SDB_PGN_RUBRO
	 */
	public void update(Rubro atd_param)
	    throws B2BException
	{
		if(atd_param != null)
		{
	 ...
	 */
	private Rubro getObjetFromResultSet(ResultSet ars_rs)
	    throws SQLException
	{
		Rubro lc_datos;
		...
		lc_datos.setNumeroRubro(ars_rs.getString("NUMERO_RUBRO"));

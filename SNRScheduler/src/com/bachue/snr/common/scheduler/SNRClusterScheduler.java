...
import com.bachue.snr.ejb.session.stateless.reference.SchedulerRemote;
...
/**
 * Agendador de tareas automáticas QUARTZ
 *
 * @author Edgar Prieto
 *
 */
public abstract class SNRClusterScheduler
{
	/** Constante clh_LOGGER. */
	private static final LoggerHandler clh_LOGGER = (LoggerHandler)com.bachue.snr.prosnr01.common.utils.LoggingHelper
			.getLogger(SNRClusterScheduler.class);
	...
	/**
	 * Elimina una tarea automática del cluster al que pertenece
	 *
	...
	/**
	 * Detiene los todos los hilos de ejecución del cluster
	 *
	...
	*/
	...
	/**
	 * Inicializa los Scheduleres según la configuración
	 *
	...
	/**
	 * Inicialización de Job / Trigger para ejecución de tarea automatica según configuración
	 *
	 ...
	 * @param ac_job Clase que implementa la tarea automática a ejecutar
	 * @param as_nombreJob Identificador de la tara automatica a ejecutar
	...
	protected void iniciarJob(
	    ...
	    Class ac_job, String as_nombreJob, boolean ab_agendarJob
	)
	{
		iniciarJob(
		    amsjd_jobs, amst_triggers, null, null, null, as_cluster, as_nodo, ac_job, as_nombreJob, ab_agendarJob
		);
	}
	...
	protected void iniciarJob(
	    Map<String, JobDetail> amsjd_jobs, Map<String, Trigger> amst_triggers, String as_cluster, String as_nodo,
	    Class ac_job, String as_nombreJob
	)
	{
		iniciarJob(amsjd_jobs, amst_triggers, as_cluster, as_nodo, ac_job, as_nombreJob, false);
	}
	 ...
	 * @param as_intervalo Intervalo de ejecución del Job. Si no es suministrado el valor se toma de la parametrización
	 * @param as_cluster Cluster que se debe inicializar
	 * @param as_nodo Nodo que se está inicializando dentro del cluster
	 * @param ac_job Clase que implementa la tarea automática a ejecutar
	 * @param as_nombreJob Identificador de la tara automatica a ejecutar
	 ...
	 * @param amsjd_jobs Contenedor de Jobs donde se alojará el Job inicializado
	 * @param amst_triggers Contenedor de Triggers donde se alojará el Trigger inicializado
	 * @param ad_inicio Fecha de inicio de ejecución del Job
	 * @param as_cluster Cluster que se debe inicializar
	 * @param as_nodo Nodo que se está inicializando dentro del cluster
	 * @param ac_job Clase que implementa la tarea automática a ejecutar
	 * @param as_nombreJob Identificador de la tara automatica a ejecutar
	...
	)
	{
		...
		if(lsr_reference != null)
		{
			...
			try
			{
				...
				{
					StringBuilder lsb_idConstanteIntervalo;

					lsb_idConstanteIntervalo = new StringBuilder(Quartz.PREFIJO_JOB);

					lsb_idConstanteIntervalo.append(as_nombreJob);
					lsb_idConstanteIntervalo.append(Quartz.SUFIJO_INTERVALO);

					ls_intervalo = lsr_reference.obtenerCaracterConstante(lsb_idConstanteIntervalo.toString());
				}
				...
				if(StringUtils.isValidString(ls_intervalo))
				{
					if(ljk_jobId != null)
					{
						...
						ljb_job = newJob(ac_job).withIdentity(ljk_jobId).usingJobData(SNRJob.DATA_IP_REMOTA, as_nodo);
	...
	/**
	 * Inicialización de jobs que deben ejecutarse en un nodo
	 *
	 * @param acs_nombresJob Listado de IDs de JOBs que deben inicializarse
	 * @param amsjd_jobs Contenedor de Jobs donde se alojará el Job inicializado
	 * @param amst_triggers Contenedor de Triggers donde se alojará el Trigger inicializado
	 * @param as_cluster Cluster que se debe inicializar
	 * @param as_nodo Nodo que se está inicializando dentro del cluster
	...
	protected abstract void iniciarJobs(
	    Collection<String> acs_nombresJob, Map<String, JobDetail> amsjd_jobs, Map<String, Trigger> amst_triggers,
	 ...
	 * @param as_nombreGrupo Nombre del grupo en el cual se registrará una tarea automatica
	 */
	...
	/**
	 * Obtiene el nombre del grupo en el cual se registrará una tarea automatica
	 *
	 * @param as_cluster Cluster que se debe inicializar
	 */
	...
	/**
	 * Inicializa los schedulers en modo cluster según la configuración
	 *
	...
	/**
	 * Actualiza la variable de bloqueo del Job para permitir su ejecución
	 *
	 * @param as_nombreJob Identificador de la tara automatica a ejecutar
	 */
		...
		if((lsr_reference != null) && StringUtils.isValidString(as_nombreJob))
		{
			try
			{
...

				{
					...
					lsb_idConstanteBloqueo = new StringBuilder(Quartz.PREFIJO_JOB);

					lsb_idConstanteBloqueo.append(as_nombreJob);
					lsb_idConstanteBloqueo.append(Quartz.SUFIJO_BLOQUEO);

					ls_idConstanteBloqueo = lsb_idConstanteBloqueo.toString();
				}

				ls_bloqueo = lsr_reference.obtenerCaracterConstante(ls_idConstanteBloqueo);

				if(StringUtils.isValidString(ls_bloqueo))
					lsr_reference.updateCaracter(
					    ls_idConstanteBloqueo, EstadoCommon.N,
					    (as_nombreJob.length() > 30) ? as_nombreJob.substring(0, 30) : as_nombreJob
					);
			}
			catch(Exception le_e)
			{
				clh_LOGGER.error("unblockJobStart", le_e);
			}
		}
	}
	    ...
	    throws Exception
	{
		...
		{
			Collection<String> lcs_nombresJob;

			{
				StringBuilder lsb_nombresJob;

				lsb_nombresJob = new StringBuilder(Quartz.PREFIJO_JOB);

				lsb_nombresJob.append("%");
				lsb_nombresJob.append(Quartz.SUFIJO_CLUSTER);

				lcs_nombresJob = lsr_reference.obtenerIdConstanesPorCaracterIdLikeCaracter(
					    lsb_nombresJob.toString(), as_cluster
					);
			}

			{
				...
				iniciarJobs(lcs_nombresJob, lmsjd_jobs, lmst_triggers, as_cluster, as_nodo);
	 ...
	 *
	 * @param as_cluster Cluster que se debe inicializar
	 * @param ai_cantidadJobs Cantidad de Jobs que se ejecutarán dentro del clustes
	 */
	    ...
	    throws Exception
	{
		...
		if((lsr_reference != null) && StringUtils.isValidString(as_cluster))
		{
			...
			if(ls_scheduler == null)
			{
				...
				lp_p.put(
				    "org.quartz.dataSource.snrQuartzDS.jndiURL",
				...
				lp_p.put("org.quartz.jobStore.dataSource", "snrQuartzDS");
				lp_p.put("org.quartz.jobStore.driverDelegateClass", "org.quartz.impl.jdbcjobstore.WebLogicDelegate");
